...
def compute_velocity_and_get_target_timestep(source_latents, intermediate_latents, timestep_to_infer):
    ...
    timestep_to_infer = timestep_list[timestep_to_infer]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # handle timesteps, Currently same timesteps for all images in the batch
                timesteps = torch.tensor([batch["timestep_to_infer"][0]], device=accelerator.device)
                ...
                loss = torch.mean((model_pred.float() - batch["velocity"].float()) ** 2)
                # loss = torch.mean(torch.abs(model_pred.float() - batch["velocity"].float()))
