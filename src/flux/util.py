...
class ModelSpec:
    ...
    repo_flow: str | None
    repo_ae: str | None
    diffusers_ckpt_path: str | None = None
...
configs = {
    ...
    "flux-schnell": ModelSpec(
        ...
        diffusers_ckpt_path="black-forest-labs/FLUX.1-schnell",
        params=FluxParams(
            in_channels=64,
            vec_in_dim=768,
            context_in_dim=4096,
...
def load_flow_model(name: str, device: str | torch.device = "cuda", hf_download: bool = True, model_backend: str = "blackforest"):
    ...
    else:  # diffusers backend
        from diffusers.models import FluxTransformer2DModel
        
        # Get diffusers checkpoint path
        diffusers_ckpt_path = configs[name].diffusers_ckpt_path
        if diffusers_ckpt_path is None:
            diffusers_ckpt_path = "black-forest-labs/FLUX.1-dev"
        
        print(f"Loading diffusers model from: {diffusers_ckpt_path}")
