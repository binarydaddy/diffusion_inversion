...
class RFInversionFluxPipeline(
    ...
    def invert(
        ...
        # Distributionize the image latents
        image_latents = torch.randn_like(image_latents) * 0.01 + image_latents
        ...
        timestep_backward = torch.flip(timesteps, dims=[0])
        sigmas_backward = torch.flip(sigmas, dims=[0])

        # forward ODE loop
        with self.progress_bar(total=N - 1) as progress_bar:
            for i in range(N - 1):
                t_i = torch.tensor(sigmas_backward[i+1], dtype=Y_t.dtype, device=device)
                ...
                Y_t = Y_t + u_hat_t_i * (sigmas_backward[i+1] - sigmas_backward[i])
                progress_bar.update()

        latents_img = self._unpack_latents(Y_t, height, width, self.vae_scale_factor)
        latents_img = (latents_img / self.vae.config.scaling_factor) + self.vae.config.shift_factor
        image = self.vae.decode(latents_img, return_dict=False)[0]
        image = self.image_processor.postprocess(image, output_type='pil')

        # Offload all models
        self.maybe_free_model_hooks()
        return Y_t, image_latents, latent_image_ids, image